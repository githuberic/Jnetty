package com.lgq.jnetty.cases.chapter4;
...
public class HttpClient {
    ...
    private com.lgq.jnetty.cases.chapter4.HttpResponse blockSend(FullHttpRequest request) throws InterruptedException, ExecutionException {
        request.headers().set(HttpHeaderNames.CONTENT_LENGTH, request.content().readableBytes());

        DefaultPromise<com.lgq.jnetty.cases.chapter4.HttpResponse> respPromise = new DefaultPromise<>(channel.eventLoop());
        handler.setRespPromise(respPromise);
        channel.writeAndFlush(request);
        // 阻塞调用方线程
        com.lgq.jnetty.cases.chapter4.HttpResponse response = respPromise.get();
        if (response != null) {
